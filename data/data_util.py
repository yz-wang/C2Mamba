...
from temp_code.basicsr.data.transforms import mod_crop
from temp_code.basicsr.utils import img2tensor, scandir
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    Args:
        folders (list[str]): A list of folder paths. The order should be [input_folder, gt_folder].
        keys (list[str]): A list of keys identifying folders. The order should be consistent with folders, e.g., ['lq', 'gt'].
        filename_tmpl (str): Template for each filename. The template excludes the file extension.

    Returns:
        list[dict]: A list of dictionaries containing paired paths.
    """
    assert len(folders) == 2, (
        'The len of folders should be 2 with [input_folder, gt_folder]. '
        f'But got {len(folders)}')
    ...
    # Get all paths for GT and input (LQ)
    gt_paths = list(scandir(gt_folder, full_path=True))
    input_paths = list(scandir(input_folder, full_path=True))

    # Create a dictionary to map the base name (without suffix) to the full path for input (LQ)
    input_path_map = {}
    for path in input_paths:
        base_name = osp.splitext(osp.basename(path))[0]
        # Extract the numeric part of the base name for LQ (e.g., '33' from '33_hazy')
        input_key_part = base_name.split('_')[0]
        input_path_map[input_key_part] = path

    paths = []
    for gt_path in gt_paths:
        gt_base_name = osp.splitext(osp.basename(gt_path))[0]
        # Extract the numeric part of the GT base name (e.g., '33' from '33_GT')
        gt_key_part = gt_base_name.split('_')[0]

        # Find the corresponding input (LQ) path using the numeric part
        input_path = input_path_map.get(gt_key_part)
        if input_path is not None:
            paths.append({f'{input_key}_path': input_path, f'{gt_key}_path': gt_path})
        else:
            print(f"Warning: No matching input file found for GT file {gt_path}")
